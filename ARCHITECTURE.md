...
- Engine selection runs on `TranscriptionEngineVariant` (engine + mode: MLX, Local AI Server, Nova-3, Flux Live, Scribe, Scribe Realtime), not on the brand — a live mode is a different runtime path, while backup candidates are restricted to file-transcription variants. `EngineFailoverPolicy` owns the "which engine runs" decision as pure values; keep it there and tested, never re-derived at call sites.
- The optional backup engine (Settings → Engine) covers three points, not one: it STARTS the dictation when the primary is unusable, offline, or in `EngineReachabilityLog` (the backup records locally and transcribes the complete file); it rescues a failed batch transcription; and it rescues a failed live one from the preserved WAV. Provider failures, including rejected requests/models, missing configuration, authentication, quota, rate limits and decoder limits, are eligible for one configured backup. Cancelled, interrupted, empty or invalid captures are excluded. Unknown failures are rescuable only after capture; startup requires a classified provider failure. Only `network`, `timedOut`, and `serverError` mark an engine temporarily unreachable. History records the engine that actually transcribed, and backup selection never silently switches between realtime and file models. Stored or imported realtime backup identifiers normalize to the same provider’s file variant. Explicit engine choices (history retranscribe menu) never fall back.
...
- Local AI Server availability and Settings status share ViewModel ownership. Configuration changes and successful connection tests invalidate stale cooldowns for the next take without changing an active capture. Revision-scoped observations prevent old checks from overwriting newer configuration or transcription outcomes; a transport response alone never claims transcription success. Explicit connection tests take priority over background health probes; real transcription outcomes remain authoritative. URL/model bindings commit through the ViewModel synchronously, before a window can close. Transient health-check network failures receive one cancellable confirmation inside the existing three-second request budget.
...
- MLX downloads stream into per-file partials under the model directory, validate expected sizes and HTTP ranges, and publish completed files only after the writer closes. Pause preserves partials; cancellation and deletion drain the per-model writer before removing files or starting a replacement. Operation identities reject stale callbacks. The public model downloader never populates the shared Hub cache.
