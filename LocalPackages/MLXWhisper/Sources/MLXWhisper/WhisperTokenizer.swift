...
//  commit 580e952. Local change: initial-prompt support — `encodePrompt`
//  plus an optional `<|startofprev|>` prefix in `buildPromptTokens`,
//  mirroring mlx_whisper's Python decoding prefix.
//
...
public final class WhisperTokenizer {
    ...
    public init(
        ...
        self.prevSotId =
            generationConfig?.prevSotTokenId
            ?? Self.tokenId(in: addedTokens, name: "<|startofprev|>")

        self.transcribeId =
            generationConfig?.taskToId?["transcribe"]
            ?? Self.tokenId(in: addedTokens, name: "<|transcribe|>")
        self.translateId =
            generationConfig?.taskToId?["translate"]
    ...
    /// multilingual variants auto-detect. `languageTokenId` (from the model's
    /// detection pass) wins over `language`. `initialPromptTokens` (already
    /// encoded, see `encodePrompt`) are prepended behind `<|startofprev|>`
    /// so they bias decoding without appearing in the output — the same
    /// contract as mlx_whisper / openai-whisper `initial_prompt`.
    public func buildPromptTokens(
        ...
        var tokens: [Int] = []
        if !initialPromptTokens.isEmpty, let prevSotId {
            tokens.append(prevSotId)
            tokens.append(contentsOf: initialPromptTokens)
        }
        tokens.append(startOfTranscriptId)
        if isMultilingual {
            if let languageTokenId {
                tokens.append(languageTokenId)
            } else if let resolved = resolveLanguage(language) {
    ...
    /// Encode an initial-prompt string for `buildPromptTokens`, capped to
    /// half the decoder context minus one (openai-whisper keeps the LAST
    /// tokens when the prompt is too long; mirrored here).
    public func encodePrompt(_ text: String, maxTargetPositions: Int) -> [Int] {
        let trimmed = text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return [] }
        // Leading space matches the reference implementations: the prompt is
        // treated as trailing context of a previous segment.
        let encoded = inner.encode(text: " " + trimmed).filter { !specialTokenIds.contains($0) }
        let maxPromptTokens = max(0, maxTargetPositions / 2 - 1)
        return Array(encoded.suffix(maxPromptTokens))
    }
