...
- Public local-STT fixtures live in `TestAssets/LocalAITranscription/`: longform clips under `longform/` and scored technical clips under `technical/en/` and `technical/es/` with `technical/vocabulary.json`. Use `scripts/local_stt_benchmark.sh` for quick reproducible checks without storing benchmark results in the repo.
