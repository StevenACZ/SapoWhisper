...
- AI polish: optional OpenAI-compatible provider after any engine (`OpenAICompatiblePolisher`; OpenRouter default, Local Server preset for LAN `chat/completions`, key optional when local). Local `AIPolishMemoryManager` stores compact term rankings and reviewable correction suggestions in app support; never auto-add learned corrections to vocabulary without user acceptance. A fidelity guard pastes raw when output drifts; keep `polishing` visually distinct.
