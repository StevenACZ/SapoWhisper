...
- Fidelity guard anchors are typed and intentionally narrow: numbers, URLs, emails, vocabulary, and identifier-like capitalized tokens must survive (numeric/URL/email/vocabulary punctuation is semantic — `5.5` ≠ `55`); numeric anchors match a whole numeric token, never a substring, so `5` is not satisfied by `15`/`5.5`/`5,000`/`5:30`, and short transcripts still require the raw's numbers as an ordered, duplicate-aware subsequence (a swap `5↔6` or a changed duplicate `5+5`→`5+15` fails), with digits embedded in URLs/emails excluded from the numeric sweep (the link keeps its own literal anchor). Long same-language narrative dictations can tolerate a small numeric miss only when there are at least two and at most eight numeric anchors, not all numbers are lost, and the output stays above the long-narrative ratio floor; this is for duration/estimate cleanup, not URL/email/vocabulary drift. Identifier-like capitalized tokens (`AGENTS..md`, `API`, `REST`, `SapoWhisper`) are matched punctuation-insensitively via a ≥3-alphanumeric key, so a polish that fixes a dictation typo (`AGENTS..md` → `AGENTS.md`) is not falsely rejected while dropping the word's content (→ `AGENTS`) still fails. Do not revert identifier anchors to literal `contains`, and do not extend punctuation tolerance to numeric/URL/email/vocabulary anchors.
