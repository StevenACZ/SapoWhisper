...
- AI polish: optional OpenAI-compatible provider after any engine (`OpenAICompatiblePolisher`; OpenRouter default, Local Server preset for LAN `chat/completions`, key optional when local). Local `AIPolishMemoryManager` stores only reviewable correction suggestions in app support; it must never learn, recommend, or auto-add vocabulary/keyterms. Accepted AI suggestions become normal automatic corrections, and only accepted corrections may feed future polish context. A fidelity guard pastes raw when output drifts; keep `polishing` visually distinct.
